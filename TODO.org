...
* Things to Do
Copy across smartscan-symbol and modify to use the same method as isearch does
    when looking for symbols.
    I prefer the matches that isearch gets to those of smartscan, but the
    interface of smartscan is better.
